...
//#region node_modules/.nitro/vite/services/ssr/assets/compliance-oracle.server-DNsSDuCa.js
var aiInstance = null;
function getGenAI() {
...
async function askComplianceOracle(question, history = []) {
	const ai = getGenAI();
...
${WORLD_LAWS_DATA.map((law) => `[${law.country.toUpperCase()} - ${law.acronym}]
...
Core Mandates: ${law.corePrinciples.join("; ")}`).join("\n\n")}
...
4. MULTI-JURISDICTION COMPARISONS: If asked about cross-border differences (e.g. EU GDPR vs India DPDP vs UAE PDPL vs CPRA), provide side-by-side contrast of fines, breach reporting deadlines, and consent thresholds.`;
	if (!ai) {
		...
		let relevantLaw = WORLD_LAWS_DATA.find((l) => qLower.includes(l.country.toLowerCase()) || qLower.includes(l.acronym.toLowerCase()) || qLower.includes(l.countryCode.toLowerCase()));
		if (!relevantLaw) relevantLaw = WORLD_LAWS_DATA[0];
		return {
	...
	try {
		...
		let text = "";
		let lastError;
		for (const cand of candidateModels) try {
			...
			}), cand.timeoutMs, `Model ${cand.name} timed out after ${(cand.timeoutMs / 1e3).toFixed(1)}s`);
			...
			console.warn(`[Oracle Model Failover] Candidate ${cand.name} note:`, errStr.slice(0, 100));
			if (isQuota) break;
		}
		if (!text) throw lastError || /* @__PURE__ */ new Error("Failed to generate response from intelligence models.");
		const sources = WORLD_LAWS_DATA.filter((l) => text.toLowerCase().includes(l.acronym.toLowerCase()) || text.toLowerCase().includes(l.country.toLowerCase())).slice(0, 3).map((l) => ({
			law: l.acronym,
